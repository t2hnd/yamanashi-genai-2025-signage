...
export default function SignagePage() {
  ...
  return (
    <div className="signage-container flex flex-col bg-gray-50">
      ...
      {/* 時間帯テーマ */}
      <div className="bg-gray-100 py-3 px-6 flex items-center justify-center gap-3">
        <Clock className="w-5 h-5 text-slate-600" />
        <span className="text-xl font-bold text-slate-700">{timeSlot.theme}</span>
        <span className="text-lg ml-4 text-slate-500">{timeSlot.catchphrase}</span>
      </div>
      ...
      <main className="flex-1 p-6 flex gap-6 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {mainProduct && (
            <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col animate-fade-in max-w-3xl h-full">
              <div className="flex justify-between items-start mb-2 flex-shrink-0">
                ...
                {showScores && (
                  <div className="text-right">
                    <div className="text-2xl font-bold text-orange-500">{mainProduct.score}点</div>
              ...
              <h2 className="text-3xl font-bold text-slate-700 mb-2 flex-shrink-0">
                {mainProduct.product.name}
              </h2>
              <p className="text-slate-500 mb-3 flex-shrink-0">{mainProduct.product.description}</p>
                ...
                <div className="bg-gray-50 rounded-lg p-3 mb-3 text-sm flex-shrink-0">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-500">季節:</span>
                      <div className="flex-1 score-bar">
                        <div
                          className="score-bar-fill bg-orange-400"
              ...
              <div className="flex justify-between items-end flex-shrink-0">
                <span className="price-text text-4xl text-orange-500">
                  ¥{mainProduct.product.price.toLocaleString()}
                </span>
                {showScores && (
                  <span className="text-sm text-slate-500 bg-gray-100 px-3 py-1 rounded-full">
                    {mainProduct.reason}
                  </span>
                )}
              </div>
        ...
        <div className="w-80 flex flex-col gap-4">
          <h3 className="text-lg font-bold text-slate-700">こちらもおすすめ</h3>
          {subProducts.map((rec, index) => (
            <div
              key={rec.product.code}
              className="bg-white rounded-xl shadow-md p-4 product-card animate-slide-in"
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h4 className="font-bold text-slate-700">{rec.product.name}</h4>
                    {showScores && (
      ...
      {/* フッター */}
      <footer className="bg-slate-700 text-white py-3 px-6 flex justify-between items-center">
